...
export class Visual implements IVisual {
  ...
  // ⚠️ TESTING ONLY — set to false before AppSource submission
  ...
  constructor(options: VisualConstructorOptions | undefined) {
    ...
    // ⚠️ TESTING ONLY — force Pro mode. Remove before AppSource submission.
    this.isPro = true;

    this.licenseManager = this.host.licenseManager;
    // TODO: uncomment before AppSource submission
  ...
  private renderGrid(viewport: powerbi.IViewport, useHex: boolean, isHC: boolean): void {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        if (!hasData && !inMask) continue;
        if (!hasData && !showEmpty) continue;
  ...
  private renderLegend(viewport: powerbi.IViewport, isHC: boolean): void {
    ...
    if (vals.length > 0) {
      ...
      this.addLegendLabel(this.formatValue(Math.min(...vals)), x,         y + 26, "start", lc);
      this.addLegendLabel(this.formatValue(Math.max(...vals)), x + gradW, y + 26, "end",   lc);
    }
  }
